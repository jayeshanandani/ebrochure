<div class="tasks index">
	<h2><?php echo __('Tasks'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('institute_id'); ?></th>
			<th><?php echo $this->Paginator->sort('task'); ?></th>
			<!--<th><?php echo $this->Paginator->sort('created'); ?></th>-->
			<th><?php echo $this->Paginator->sort('creator_id'); ?></th>
			<!--<th><?php echo $this->Paginator->sort('modified'); ?></th>-->
			<th><?php echo $this->Paginator->sort('modifier_id'); ?></th>
			<th><?php echo $this->Paginator->sort('title'); ?></th>
			<th><?php echo $this->Paginator->sort('status'); ?></th>
			<!--<th><?php echo $this->Paginator->sort('isActive'); ?></th>-->
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($tasks as $task): ?>
	<tr>
		<td><?php echo h($task['Task']['id']); ?>&nbsp;</td>
		<td><?php echo $this->Html->link($task['Institute']['name'], array('controller' => 'institutes', 'action' => 'view', $task['Institute']['id'])); ?></td>
		<td><?php echo h($task['Task']['task']); ?>&nbsp;</td>
	<!--	<td><?php echo h($task['Task']['created']); ?>&nbsp;</td>-->
		<td><?php echo h($task['Task']['creator_id']); ?>&nbsp;</td>
		<!--<td><?php echo h($task['Task']['modified']); ?>&nbsp;</td>-->
		<td><?php echo h($task['Task']['modifier_id']); ?>&nbsp;</td>
		<td><?php echo h($task['Task']['title']); ?>&nbsp;</td>
		<td><?php echo h($task['Task']['status']); ?>&nbsp;</td>
		<!--<td><?php echo h($task['Task']['isActive']); ?>&nbsp;</td>-->
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $task['Task']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $task['Task']['id'])); ?>
			<?php 
            if($task['Task']['isActive'] == 1){
                echo $this->Form->postLink(__('Deactive'), array('action' => 'deactivate', $task['Task']['id']), null, __('Are you sure you want to deactivate # %s?', $task['Task']['id'])); 
            }
        ?>
        <?php 
            if($task['Task']['isActive'] == 0){
                echo $this->Form->postLink(__('Active'), array('action' => 'activate', $task['Task']['id']), null, __('Are you sure you want to activate # %s?', $task['Task']['id'])); 
            }
        ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Task'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('Institutes'), array('controller' => 'institutes', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('My Dashboard'), array('controller' => 'Users', 'action' => 'user_info')); ?> </li>
	</ul>
</div>
